<script setup>
import { ref } from 'vue'
import { artEditChannelServece, artAddChannelservece } from '@/api/article';
...
const open = async (row) => {
    ...
    console.log(row)
    formModel.value = { ...row }//添加->重置了表单内容
}
const formModel = ref({
    cate_name: '',
    cate_alias: ''
...
const onSubmit = async () => {
    ...
    if (isEdit) {
        await artEditChannelServece(formModel.value)
        ElMessage.success('编辑成功')
    } else {
        await artAddChannelservece(formModel.value)
